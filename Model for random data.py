...
def main():
    ...
    data = pd.read_csv('randomdata.csv', sep=',')
    X = data.iloc[:, 1:]
    Y = data.label
    print(X.shape, Y.shape)  # 300X3, 300
    # Split dataset into training set and test set
    # keeping 30% data reserved for testing purpose and 70% data will be used to train and form model.
    X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=0.30,random_state=42)
    print(X_train.shape, Y_train.shape) #210x3, 210  
    print(X_test.shape, Y_test.shape)  #90x3, 103
    print("\n\n\n")
    c = []
    ...
    c6 = model_dtc.predict(X_test)
    # Model Accuracy: how often is the classifier correct?
    accuracy = accuracy_score(Y_test, c6)
    print("Accuracy for DecisionTree:", accuracy * 100, " %")
    #Compute confusion matrix to evaluate the accuracy of a classification.
    cm = confusion_matrix(Y_test, c6)
